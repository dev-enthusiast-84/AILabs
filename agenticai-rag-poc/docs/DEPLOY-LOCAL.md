...
> [← Home](README.md) · [Setup Guide](SETUP.md) · [Vercel Deployment](DEPLOY-VERCEL.md)

Three local run modes: **dev** (hot reload), **production-like build**, and **Docker Compose**.

## 1. Local Dev (Hot Reload)

**Prerequisites:** Complete [Setup Guide](SETUP.md) first — Python 3.11–3.13, Node 20+, `setup.sh` run.
...
|----------|---------|----------|---------|
| `OPENAI_API_KEY` | — | **Yes** | LLM completions and text embeddings |
| `SECRET_KEY` | insecure default | **Yes in prod** | JWT signing — `openssl rand -hex 32` |
| `ADMIN_PASSWORD` | _(auto-generated)_ | **Yes** | Login password |
| `APP_ENV` | `development` | No | `production` disables Swagger UI and credential banner |
| `SESSION_COMPATIBILITY_VERSION` | `1` | No | Bump after incompatible deploys to force admin/guest re-login |
| `VECTOR_STORE_TYPE` | `chroma` | No | `chroma` for local persistence; `pinecone` for production/Vercel; `memory` for tests |
...
After the stack is running and documents are indexed, run the live Ragas evaluation suite:
...
|----------|---------|---------|
| `RAGAS_SCORES_FILE` | `/tmp/ragas_scores.json` | Path where Ragas results are persisted; read by the admin Settings → Ragas Evaluation panel |

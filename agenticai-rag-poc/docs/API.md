...
For request/response schemas, AgentTrace, and rate limits → [API Schemas & Examples](API-SCHEMAS.md).
...
| `GET`  | `/api/settings/` | Bearer | ✅ | Active model and masked API key |
| `POST` | `/api/settings/` | Bearer | ✅ limited | Update API key (any time) or model (once per guest session) |
| `POST` | `/api/settings/ragas-trigger` | Bearer | ❌ 403 | Trigger an async Ragas evaluation run (admin only). Rate-limited to 1 per 5 min. |


- **Getting Started**
  - [Home](README.md)
  - [**Capability Showcase**](pitch.html ':ignore')
  - [Walkthrough Videos](walkthrough/index.html ':ignore')

- **Architecture**
  - [Architecture Overview](architecture/ARCHITECTURE.md)
    - [Agent Pipeline](architecture/AGENT-PIPELINE.md)
    - [Project Structure](architecture/ARCHITECTURE-STRUCTURE.md)

- **Deployment**
  - [Deployment Overview](deployment/DEPLOYMENT.md)
    - [Setup Guide](deployment/SETUP.md)
    - [Setup Verification](deployment/SETUP-VERIFY.md)
    - **— Local —**
    - [Local & Docker](deployment/DEPLOY-LOCAL.md)
    - [Environment Variables](deployment/DEPLOY-LOCAL-ENV.md)
    - [Pipeline & Retrieval Vars](deployment/DEPLOY-LOCAL-ENV-PIPELINE.md)
    - **— Remote —**
    - [Vercel Deployment](deployment/DEPLOY-VERCEL.md)
    - [Vercel Operations](deployment/DEPLOY-VERCEL-OPS.md)
    - [Operational Limits](deployment/DEPLOY-LIMITS.md)

- **API Reference**
  - [API Overview](api/API.md)
    - [Schemas & Examples](api/API-SCHEMAS.md)
    - [Document Schemas](api/API-SCHEMAS-DOCUMENTS.md)
    - [Trace Schemas](api/API-SCHEMAS-TRACE.md)
    - [Voice Export Schemas](api/API-SCHEMAS-VOICE.md)

- **Testing**
  - [Testing Overview](testing/TESTING.md)
    - [Integration Tests](testing/TESTING-INTEGRATION.md)
    - [Frontend & E2E](testing/TESTING-FRONTEND.md)
    - [Testing Challenges](testing/TESTING-CHALLENGES.md)
    - [Coverage Matrix](testing/COVERAGE-MATRIX.md)

- **Security**
  - [Security Overview](security/SECURITY.md)
    - [Content Guardrails](security/GUARDRAILS.md)
    - [Guardrails API](security/GUARDRAILS-API.md)
    - [Production Hardening](security/SECURITY-HARDENING.md)

- **Project**
  - [Project Overview](project/PROJECT.md)
    - [Capstone Audit](project/CAPSTONE-AUDIT.md)
    - [Limitations & Challenges](project/CHALLENGES.md)
      - [Retrieval & Reranking](project/CHALLENGES-RETRIEVAL.md)
      - [Agent Pipeline Challenges](project/CHALLENGES-AGENT-PIPELINE.md)
    - [Cost Analysis](project/COST-ANALYSIS.md)
      - [Cost Tracing Guide](project/COST-ANALYSIS-TRACING.md)
    - [SDD Workflow](project/SDD.md)
    - [Walkthrough Videos](project/WALKTHROUGH-VIDEO.md)
    - [Future Enhancements](project/POTENTIAL-ENTERPRISE-GRADE-FUTURE-ENHANCEMENTS.md)

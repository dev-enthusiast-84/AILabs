...
ALLOWED_RERANKER_TYPES: frozenset[str] = frozenset({"none", "cross-encoder"})
...
def get_masked_langchain_api_key() -> str:
    ...
    with _lock:
        key = _runtime_langchain_api_key or _account_env_value(get_settings().langchain_api_key)
        if not key:
            return ""
        return f"ls-****...{key[-4:]}"
...
def get_effective_pinecone_api_key() -> str:
    with _lock:
        ...
        if guest_key:
            return str(guest_key)
        if _is_guest_scope():
            return _account_env_value(get_settings().pinecone_api_key)
...
def get_effective_pinecone_cloud() -> str:
    with _lock:
        ...
        if _is_guest_scope():
            return get_settings().pinecone_cloud
...
def get_effective_pinecone_region() -> str:
    with _lock:
        ...
        if _is_guest_scope():
            return get_settings().pinecone_region
...
def get_masked_pinecone_api_key() -> str:
    ...
    with _lock:
        key = _guest_value("pinecone_api_key") or (
            _account_env_value(get_settings().pinecone_api_key) if _is_guest_scope()
            else _runtime_pinecone_api_key or _account_env_value(get_settings().pinecone_api_key)
        )
        if not key:
            return ""
...
def get_effective_blob_read_write_token() -> str:
    ...
    with _lock:
        ...
        if not account_env_fallback_allowed():
            ...
            vercel_blob_token = ""
        if _is_guest_scope():
            return str(_guest_value("blob_read_write_token") or blob_token or vercel_blob_token)
...
def get_masked_blob_read_write_token() -> str:
    ...
    with _lock:
        ...
        if not account_env_fallback_allowed():
            blob_token = ""
            vercel_blob_token = ""
        key = _guest_value("blob_read_write_token") or (
            blob_token or vercel_blob_token if _is_guest_scope()
            else _runtime_blob_read_write_token or blob_token or vercel_blob_token
        )
        if not key:
            return ""
...
def get_effective_reranker_type() -> str:
    """Return the effective reranker type ('none' or 'cross-encoder')."""
    with _lock:
        return (
            _runtime_reranker_type
            if _runtime_reranker_type is not None
            else get_settings().reranker_type
        )
